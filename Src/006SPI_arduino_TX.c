/*
 * @file		006SPI_arduino_TX_test.c
 * @author		ernad
 * @date 		Aug 12, 2026
 * @brief		3-Line SPI TX test with Arduino slave.
 *
 * @details
 * This application tests basic SPI-send functionality using an
 * an Arduino slave.
 */
...
// Initializes SPI2 peripheral for TX
static void spi2_init(SPI_handle_t *spi2_handle)
{
    ...
    SPI_PeripheralControl(SPI2, ENABLE);    // automatically makes NSS=0 when SSM=0
}
...
{    
    ...
    while(1)
    {
        ...
        SPI_Send(spi2_handle.pSPIx, (uint8_t*)(&len), 1U);  // send length first
        SPI_Send(spi2_handle.pSPIx, (uint8_t*)msg, len);    // send payload

        while( SPI_GetFlag(SPI2, SPI_SR_BSY_BIT) );         // wait until not busy
